...
The function is located at: `app/supabase/functions/analytics-kpis/index.ts`
...
Navigate to the `app` directory (where your supabase functions folder is):
...
### Alternative: Deploy from project root

If you want to deploy from the project root, you can use:

```bash
supabase functions deploy analytics-kpis --project-ref YOUR_PROJECT_REF
```

## Verify Deployment

After deployment, you should see:
- Success message with the function URL
...
curl -X POST "https://YOUR_PROJECT_REF.supabase.co/functions/v1/analytics-kpis" \
...
1. **If you get "project not linked"**: Run `supabase link --project-ref YOUR_PROJECT_REF` first
2. **If function doesn't appear**: Check that you're in the correct directory and the function folder exists at `app/supabase/functions/analytics-kpis/`
3. **If deployment fails**: Check your Supabase CLI version with `supabase --version` and update if needed
